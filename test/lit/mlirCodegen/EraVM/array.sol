// RUN: solc --mlir-action=print-std-mlir --mlir-target=eravm --mmlir --mlir-print-debuginfo %s | FileCheck %s

contract C {
  function ui_1d(uint256[3] memory a, uint256 i) private returns (uint256) {
    return a[i];
  }
  function ui_2d(uint256[3][4] memory a, uint256 i) private returns (uint256) {
    return a[i][i];
  }

  function malloc_1d() private {
    uint256[3] memory a;
  }
  function malloc_2d() private {
    uint256[3][4] memory a;
  }
}

// NOTE: Assertions have been autogenerated by test/updFileCheckTest.py
// CHECK: #NonPayable = #sol<StateMutability NonPayable>
// CHECK-NEXT: #loc = loc(unknown)
// CHECK-NEXT: #loc2 = loc({{.*}}:2:0)
// CHECK-NEXT: module {
// CHECK-NEXT:   func.func private @__return(i256, i256, i256) attributes {llvm.linkage = #llvm.linkage<external>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} loc(#loc)
// CHECK-NEXT:   func.func private @".unreachable"() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} {
// CHECK-NEXT:     llvm.unreachable loc(#loc1)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT:   func.func private @__revert(i256, i256, i256) attributes {llvm.linkage = #llvm.linkage<external>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} loc(#loc)
// CHECK-NEXT:   func.func private @__deploy() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} {
// CHECK-NEXT:     %c128_i256 = arith.constant 128 : i256 loc(#loc2)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc2)
// CHECK-NEXT:     %0 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     llvm.store %c128_i256, %0 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     %1 = "llvm.intrcall"() <{id = 3177 : i32, name = "eravm.getu128"}> : () -> i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %2 = arith.cmpi ne, %1, %c0_i256 : i256 loc(#loc2)
// CHECK-NEXT:     scf.if %2 {
// CHECK-NEXT:       %c0_i256_8 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:       %c2_i256_9 = arith.constant 2 : i256 loc(#loc2)
// CHECK-NEXT:       func.call @__revert(%c0_i256_8, %c0_i256_8, %c2_i256_9) : (i256, i256, i256) -> () loc(#loc2)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc2)
// CHECK-NEXT:     } loc(#loc2)
// CHECK-NEXT:     %c64_i256_0 = arith.constant 64 : i256 loc(#loc2)
// CHECK-NEXT:     %3 = llvm.inttoptr %c64_i256_0 : i256 to !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     %4 = llvm.load %3 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_1 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_2 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %5 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     %6 = llvm.load %5 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     %7 = llvm.getelementptr %6[%c0_i256_1] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc2)
// CHECK-NEXT:     %8 = llvm.inttoptr %4 : i256 to !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     "llvm.intr.memcpy"(%8, %7, %c0_i256_2) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc2)
// CHECK-NEXT:     %c256_i256 = arith.constant 256 : i256 loc(#loc2)
// CHECK-NEXT:     %9 = llvm.inttoptr %c256_i256 : i256 to !llvm.ptr<2> loc(#loc2)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c32_i256, %9 {alignment = 1 : i64} : i256, !llvm.ptr<2> loc(#loc2)
// CHECK-NEXT:     %c288_i256 = arith.constant 288 : i256 loc(#loc2)
// CHECK-NEXT:     %10 = llvm.inttoptr %c288_i256 : i256 to !llvm.ptr<2> loc(#loc2)
// CHECK-NEXT:     %c0_i256_3 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c0_i256_3, %10 {alignment = 1 : i64} : i256, !llvm.ptr<2> loc(#loc2)
// CHECK-NEXT:     %c0_i256_4 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c2_i256 = arith.constant 2 : i256 loc(#loc2)
// CHECK-NEXT:     %11 = arith.muli %c0_i256_4, %c2_i256 : i256 loc(#loc2)
// CHECK-NEXT:     %c64_i256_5 = arith.constant 64 : i256 loc(#loc2)
// CHECK-NEXT:     %12 = arith.addi %11, %c64_i256_5 : i256 loc(#loc2)
// CHECK-NEXT:     %c256_i256_6 = arith.constant 256 : i256 loc(#loc2)
// CHECK-NEXT:     %c2_i256_7 = arith.constant 2 : i256 loc(#loc2)
// CHECK-NEXT:     call @__return(%c256_i256_6, %12, %c2_i256_7) : (i256, i256, i256) -> () loc(#loc2)
// CHECK-NEXT:     call @".unreachable"() : () -> () loc(#loc2)
// CHECK-NEXT:     llvm.unreachable loc(#loc2)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT:   func.func private @__runtime() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, runtime} {
// CHECK-NEXT:     %c128_i256 = arith.constant 128 : i256 loc(#loc2)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc2)
// CHECK-NEXT:     %0 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     llvm.store %c128_i256, %0 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc2)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_0 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_1 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     call @__revert(%c0_i256, %c0_i256_0, %c0_i256_1) : (i256, i256, i256) -> () loc(#loc2)
// CHECK-NEXT:     call @".unreachable"() : () -> () loc(#loc2)
// CHECK-NEXT:     llvm.unreachable loc(#loc2)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @ptr_decommit() {addr_space = 0 : i32} : !llvm.ptr<3> loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @ptr_return_data() {addr_space = 0 : i32} : !llvm.ptr<3> loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @ptr_calldata() {addr_space = 0 : i32} : !llvm.ptr<3> loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @ptr_active() {addr_space = 0 : i32} : !llvm.array<16 x ptr<3>> loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @extra_abi_data(dense<0> : tensor<10xi256>) {addr_space = 0 : i32} : !llvm.array<10 x i256> loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @call_flags(0 : i256) {addr_space = 0 : i32} : i256 loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @returndatasize(0 : i256) {addr_space = 0 : i32} : i256 loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @calldatasize(0 : i256) {addr_space = 0 : i32} : i256 loc(#loc)
// CHECK-NEXT:   llvm.mlir.global private @memory_pointer(0 : i256) {addr_space = 0 : i32} : i256 loc(#loc)
// CHECK-NEXT:   func.func private @__entry(%arg0: !llvm.ptr<3> loc({{.*}}:2:0), %arg1: i256 loc({{.*}}:2:0), %arg2: i256 loc({{.*}}:2:0), %arg3: i256 loc({{.*}}:2:0), %arg4: i256 loc({{.*}}:2:0), %arg5: i256 loc({{.*}}:2:0), %arg6: i256 loc({{.*}}:2:0), %arg7: i256 loc({{.*}}:2:0), %arg8: i256 loc({{.*}}:2:0), %arg9: i256 loc({{.*}}:2:0), %arg10: i256 loc({{.*}}:2:0), %arg11: i256 loc({{.*}}:2:0)) -> i256 attributes {llvm.linkage = #llvm.linkage<external>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} {
// CHECK-NEXT:     %0 = llvm.mlir.addressof @memory_pointer : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c0_i256, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %1 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %c0_i256_0 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c0_i256_0, %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %2 = llvm.mlir.addressof @returndatasize : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %c0_i256_1 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c0_i256_1, %2 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %3 = llvm.mlir.addressof @call_flags : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %c0_i256_2 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     llvm.store %c0_i256_2, %3 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %4 = llvm.mlir.addressof @extra_abi_data : !llvm.ptr<array<10 x i256>> loc(#loc2)
// CHECK-NEXT:     %5 = llvm.mlir.constant(dense<0> : vector<10xi256>) : vector<10xi256> loc(#loc2)
// CHECK-NEXT:     llvm.store %5, %4 : !llvm.ptr<array<10 x i256>> loc(#loc2)
// CHECK-NEXT:     %6 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg0, %6 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     %7 = llvm.ptrtoint %6 : !llvm.ptr<ptr<3>> to i256 loc(#loc2)
// CHECK-NEXT:     %c96_i256 = arith.constant 96 : i256 loc(#loc2)
// CHECK-NEXT:     %8 = llvm.lshr %7, %c96_i256  : i256 loc(#loc2)
// CHECK-NEXT:     %c4294967295_i256 = arith.constant 4294967295 : i256 loc(#loc2)
// CHECK-NEXT:     %9 = llvm.and %8, %c4294967295_i256  : i256 loc(#loc2)
// CHECK-NEXT:     %10 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %9, %10 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %11 = llvm.load %10 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %12 = llvm.getelementptr %arg0[%11] : (!llvm.ptr<3>, i256) -> !llvm.ptr, i8 loc(#loc2)
// CHECK-NEXT:     %13 = llvm.mlir.addressof @ptr_return_data : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %13 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     %14 = llvm.mlir.addressof @ptr_decommit : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %14 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc2)
// CHECK-NEXT:     %15 = llvm.mlir.addressof @ptr_active : !llvm.ptr<array<16 x ptr<3>>> loc(#loc2)
// CHECK-NEXT:     %c0_i256_3 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_4 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %16 = llvm.getelementptr %15[%c0_i256_3, %c0_i256_4] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %16 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_5 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc2)
// CHECK-NEXT:     %17 = llvm.getelementptr %15[%c0_i256_5, %c1_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %17 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_6 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c2_i256 = arith.constant 2 : i256 loc(#loc2)
// CHECK-NEXT:     %18 = llvm.getelementptr %15[%c0_i256_6, %c2_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %18 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_7 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c3_i256 = arith.constant 3 : i256 loc(#loc2)
// CHECK-NEXT:     %19 = llvm.getelementptr %15[%c0_i256_7, %c3_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %19 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_8 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c4_i256 = arith.constant 4 : i256 loc(#loc2)
// CHECK-NEXT:     %20 = llvm.getelementptr %15[%c0_i256_8, %c4_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %20 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_9 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c5_i256 = arith.constant 5 : i256 loc(#loc2)
// CHECK-NEXT:     %21 = llvm.getelementptr %15[%c0_i256_9, %c5_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %21 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_10 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c6_i256 = arith.constant 6 : i256 loc(#loc2)
// CHECK-NEXT:     %22 = llvm.getelementptr %15[%c0_i256_10, %c6_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %22 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_11 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c7_i256 = arith.constant 7 : i256 loc(#loc2)
// CHECK-NEXT:     %23 = llvm.getelementptr %15[%c0_i256_11, %c7_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %23 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_12 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c8_i256 = arith.constant 8 : i256 loc(#loc2)
// CHECK-NEXT:     %24 = llvm.getelementptr %15[%c0_i256_12, %c8_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %24 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_13 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c9_i256 = arith.constant 9 : i256 loc(#loc2)
// CHECK-NEXT:     %25 = llvm.getelementptr %15[%c0_i256_13, %c9_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %25 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_14 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c10_i256 = arith.constant 10 : i256 loc(#loc2)
// CHECK-NEXT:     %26 = llvm.getelementptr %15[%c0_i256_14, %c10_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %26 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_15 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c11_i256 = arith.constant 11 : i256 loc(#loc2)
// CHECK-NEXT:     %27 = llvm.getelementptr %15[%c0_i256_15, %c11_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %27 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_16 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c12_i256 = arith.constant 12 : i256 loc(#loc2)
// CHECK-NEXT:     %28 = llvm.getelementptr %15[%c0_i256_16, %c12_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %28 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_17 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c13_i256 = arith.constant 13 : i256 loc(#loc2)
// CHECK-NEXT:     %29 = llvm.getelementptr %15[%c0_i256_17, %c13_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %29 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_18 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c14_i256 = arith.constant 14 : i256 loc(#loc2)
// CHECK-NEXT:     %30 = llvm.getelementptr %15[%c0_i256_18, %c14_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %30 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %c0_i256_19 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c15_i256 = arith.constant 15 : i256 loc(#loc2)
// CHECK-NEXT:     %31 = llvm.getelementptr %15[%c0_i256_19, %c15_i256] : (!llvm.ptr<array<16 x ptr<3>>>, i256, i256) -> !llvm.ptr<3>, !llvm.array<16 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %12, %31 {alignment = 32 : i64} : !llvm.ptr, !llvm.ptr<3> loc(#loc2)
// CHECK-NEXT:     %32 = llvm.mlir.addressof @call_flags : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg1, %32 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc2)
// CHECK-NEXT:     %33 = llvm.mlir.addressof @extra_abi_data : !llvm.ptr<array<10 x i256>> loc(#loc2)
// CHECK-NEXT:     %c0_i256_20 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c0_i256_21 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %34 = llvm.getelementptr %33[%c0_i256_20, %c0_i256_21] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg2, %34 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_22 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c1_i256_23 = arith.constant 1 : i256 loc(#loc2)
// CHECK-NEXT:     %35 = llvm.getelementptr %33[%c0_i256_22, %c1_i256_23] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg3, %35 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_24 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c2_i256_25 = arith.constant 2 : i256 loc(#loc2)
// CHECK-NEXT:     %36 = llvm.getelementptr %33[%c0_i256_24, %c2_i256_25] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg4, %36 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_26 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c3_i256_27 = arith.constant 3 : i256 loc(#loc2)
// CHECK-NEXT:     %37 = llvm.getelementptr %33[%c0_i256_26, %c3_i256_27] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg5, %37 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_28 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c4_i256_29 = arith.constant 4 : i256 loc(#loc2)
// CHECK-NEXT:     %38 = llvm.getelementptr %33[%c0_i256_28, %c4_i256_29] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg6, %38 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_30 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c5_i256_31 = arith.constant 5 : i256 loc(#loc2)
// CHECK-NEXT:     %39 = llvm.getelementptr %33[%c0_i256_30, %c5_i256_31] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg7, %39 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_32 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c6_i256_33 = arith.constant 6 : i256 loc(#loc2)
// CHECK-NEXT:     %40 = llvm.getelementptr %33[%c0_i256_32, %c6_i256_33] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg8, %40 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_34 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c7_i256_35 = arith.constant 7 : i256 loc(#loc2)
// CHECK-NEXT:     %41 = llvm.getelementptr %33[%c0_i256_34, %c7_i256_35] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg9, %41 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_36 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c8_i256_37 = arith.constant 8 : i256 loc(#loc2)
// CHECK-NEXT:     %42 = llvm.getelementptr %33[%c0_i256_36, %c8_i256_37] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg10, %42 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c0_i256_38 = arith.constant 0 : i256 loc(#loc2)
// CHECK-NEXT:     %c9_i256_39 = arith.constant 9 : i256 loc(#loc2)
// CHECK-NEXT:     %43 = llvm.getelementptr %33[%c0_i256_38, %c9_i256_39] : (!llvm.ptr<array<10 x i256>>, i256, i256) -> !llvm.ptr, !llvm.array<10 x i256> loc(#loc2)
// CHECK-NEXT:     llvm.store %arg11, %43 {alignment = 32 : i64} : i256, !llvm.ptr loc(#loc2)
// CHECK-NEXT:     %c1_i256_40 = arith.constant 1 : i256 loc(#loc2)
// CHECK-NEXT:     %44 = arith.andi %arg1, %c1_i256_40 : i256 loc(#loc2)
// CHECK-NEXT:     %c1_i256_41 = arith.constant 1 : i256 loc(#loc2)
// CHECK-NEXT:     %45 = arith.cmpi eq, %44, %c1_i256_41 : i256 loc(#loc2)
// CHECK-NEXT:     scf.if %45 {
// CHECK-NEXT:       func.call @__deploy() : () -> () loc(#loc2)
// CHECK-NEXT:     } else {
// CHECK-NEXT:       func.call @__runtime() : () -> () loc(#loc2)
// CHECK-NEXT:     } loc(#loc2)
// CHECK-NEXT:     llvm.unreachable loc(#loc2)
// CHECK-NEXT:   } loc(#loc)
// CHECK-NEXT:   func.func @malloc_2d_60.0() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc4)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %c128_i256 = arith.constant 128 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %1 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %2 = llvm.load %1 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %3 = arith.addi %2, %c128_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %4 = arith.cmpi ugt, %3, %c18446744073709551615_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %5 = arith.cmpi ult, %3, %2 : i256 loc(#loc4)
// CHECK-NEXT:     %6 = arith.ori %4, %5 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %6 {
// CHECK-NEXT:       %c0_i256_21 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256_21 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_22 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_22, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_0 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %7 = llvm.inttoptr %c64_i256_0 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %3, %7 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c96_i256 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_1 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %8 = llvm.inttoptr %c64_i256_1 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %9 = llvm.load %8 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %10 = arith.addi %9, %c96_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_2 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %11 = arith.cmpi ugt, %10, %c18446744073709551615_i256_2 : i256 loc(#loc4)
// CHECK-NEXT:     %12 = arith.cmpi ult, %10, %9 : i256 loc(#loc4)
// CHECK-NEXT:     %13 = arith.ori %11, %12 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %13 {
// CHECK-NEXT:       %c0_i256_21 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256_21 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_22 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_22, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_3 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %14 = llvm.inttoptr %c64_i256_3 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %10, %14 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:     %15 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %16 = llvm.load %15 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %17 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %18 = llvm.load %17 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %19 = llvm.getelementptr %18[%16] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %20 = llvm.inttoptr %9 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%20, %19, %c96_i256) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %21 = llvm.inttoptr %2 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %9, %21 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc4)
// CHECK-NEXT:     %22 = arith.addi %2, %c32_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_4 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_5 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %23 = llvm.inttoptr %c64_i256_5 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %24 = llvm.load %23 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %25 = arith.addi %24, %c96_i256_4 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_6 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %26 = arith.cmpi ugt, %25, %c18446744073709551615_i256_6 : i256 loc(#loc4)
// CHECK-NEXT:     %27 = arith.cmpi ult, %25, %24 : i256 loc(#loc4)
// CHECK-NEXT:     %28 = arith.ori %26, %27 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %28 {
// CHECK-NEXT:       %c0_i256_21 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256_21 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_22 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_22, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_7 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %29 = llvm.inttoptr %c64_i256_7 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %25, %29 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c0_i256_8 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:     %30 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %31 = llvm.load %30 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %32 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %33 = llvm.load %32 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %34 = llvm.getelementptr %33[%31] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %35 = llvm.inttoptr %24 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%35, %34, %c96_i256_4) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %36 = llvm.inttoptr %22 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %24, %36 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c64_i256_9 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %37 = arith.addi %2, %c64_i256_9 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_10 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_11 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %38 = llvm.inttoptr %c64_i256_11 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %39 = llvm.load %38 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %40 = arith.addi %39, %c96_i256_10 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_12 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %41 = arith.cmpi ugt, %40, %c18446744073709551615_i256_12 : i256 loc(#loc4)
// CHECK-NEXT:     %42 = arith.cmpi ult, %40, %39 : i256 loc(#loc4)
// CHECK-NEXT:     %43 = arith.ori %41, %42 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %43 {
// CHECK-NEXT:       %c0_i256_21 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256_21 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_22 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_22, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_13 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %44 = llvm.inttoptr %c64_i256_13 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %40, %44 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c0_i256_14 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:     %45 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %46 = llvm.load %45 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %47 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %48 = llvm.load %47 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %49 = llvm.getelementptr %48[%46] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %50 = llvm.inttoptr %39 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%50, %49, %c96_i256_10) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %51 = llvm.inttoptr %37 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %39, %51 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c96_i256_15 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %52 = arith.addi %2, %c96_i256_15 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_16 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_17 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %53 = llvm.inttoptr %c64_i256_17 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %54 = llvm.load %53 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %55 = arith.addi %54, %c96_i256_16 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_18 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %56 = arith.cmpi ugt, %55, %c18446744073709551615_i256_18 : i256 loc(#loc4)
// CHECK-NEXT:     %57 = arith.cmpi ult, %55, %54 : i256 loc(#loc4)
// CHECK-NEXT:     %58 = arith.ori %56, %57 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %58 {
// CHECK-NEXT:       %c0_i256_21 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256_21 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_22 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_22, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_19 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %59 = llvm.inttoptr %c64_i256_19 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %55, %59 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c0_i256_20 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:     %60 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %61 = llvm.load %60 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %62 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %63 = llvm.load %62 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %64 = llvm.getelementptr %63[%61] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %65 = llvm.inttoptr %54 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%65, %64, %c96_i256_16) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %66 = llvm.inttoptr %52 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %54, %66 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %2, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     return loc(#loc3)
// CHECK-NEXT:   } loc(#loc3)
// CHECK-NEXT:   func.func @malloc_1d_47.0() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc6)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %c96_i256 = arith.constant 96 : i256 loc(#loc6)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc6)
// CHECK-NEXT:     %1 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     %2 = llvm.load %1 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc6)
// CHECK-NEXT:     %3 = arith.addi %2, %c96_i256 : i256 loc(#loc6)
// CHECK-NEXT:     %c18446744073709551615_i256 = arith.constant 18446744073709551615 : i256 loc(#loc6)
// CHECK-NEXT:     %4 = arith.cmpi ugt, %3, %c18446744073709551615_i256 : i256 loc(#loc6)
// CHECK-NEXT:     %5 = arith.cmpi ult, %3, %2 : i256 loc(#loc6)
// CHECK-NEXT:     %6 = arith.ori %4, %5 : i1 loc(#loc6)
// CHECK-NEXT:     scf.if %6 {
// CHECK-NEXT:       %c0_i256_1 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc6)
// CHECK-NEXT:       %14 = llvm.inttoptr %c0_i256_1 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %14 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc6)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc6)
// CHECK-NEXT:       %15 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       llvm.store %c65_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       %c0_i256_2 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc6)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc6)
// CHECK-NEXT:       func.call @__revert(%c0_i256_2, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc6)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc6)
// CHECK-NEXT:     } loc(#loc6)
// CHECK-NEXT:     %c64_i256_0 = arith.constant 64 : i256 loc(#loc6)
// CHECK-NEXT:     %7 = llvm.inttoptr %c64_i256_0 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     llvm.store %3, %7 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     %c0_i256 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:     %8 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %9 = llvm.load %8 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %10 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc6)
// CHECK-NEXT:     %11 = llvm.load %10 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc6)
// CHECK-NEXT:     %12 = llvm.getelementptr %11[%9] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc6)
// CHECK-NEXT:     %13 = llvm.inttoptr %2 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     "llvm.intr.memcpy"(%13, %12, %c96_i256) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc6)
// CHECK-NEXT:     llvm.store %2, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     return loc(#loc5)
// CHECK-NEXT:   } loc(#loc5)
// CHECK-NEXT:   func.func @ui_2d_36.0(%arg0: i256 loc(unknown), %arg1: i256 loc(unknown)) -> i256 attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc8)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc8)
// CHECK-NEXT:     llvm.store %arg0, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc8)
// CHECK-NEXT:     %c1_i256_0 = arith.constant 1 : i256 loc(#loc9)
// CHECK-NEXT:     %1 = llvm.alloca %c1_i256_0 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc9)
// CHECK-NEXT:     llvm.store %arg1, %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc9)
// CHECK-NEXT:     %2 = llvm.load %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc10)
// CHECK-NEXT:     %3 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc11)
// CHECK-NEXT:     %c4_i256 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:     %4 = arith.cmpi uge, %3, %c4_i256 : i256 loc(#loc10)
// CHECK-NEXT:     scf.if %4 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc10)
// CHECK-NEXT:       %15 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c4_i256_2 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc10)
// CHECK-NEXT:       %16 = llvm.inttoptr %c4_i256_2 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c50_i256, %16 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c0_i256_3 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc10)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc10)
// CHECK-NEXT:       func.call @__revert(%c0_i256_3, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc10)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc10)
// CHECK-NEXT:     } loc(#loc10)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc10)
// CHECK-NEXT:     %5 = arith.muli %3, %c32_i256 : i256 loc(#loc10)
// CHECK-NEXT:     %6 = arith.addi %2, %5 : i256 loc(#loc10)
// CHECK-NEXT:     %7 = llvm.inttoptr %6 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:     %8 = llvm.load %7 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc10)
// CHECK-NEXT:     %9 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc12)
// CHECK-NEXT:     %c3_i256 = arith.constant 3 : i256 loc(#loc10)
// CHECK-NEXT:     %10 = arith.cmpi uge, %9, %c3_i256 : i256 loc(#loc10)
// CHECK-NEXT:     scf.if %10 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc10)
// CHECK-NEXT:       %15 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c4_i256_2 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc10)
// CHECK-NEXT:       %16 = llvm.inttoptr %c4_i256_2 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c50_i256, %16 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c0_i256_3 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc10)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc10)
// CHECK-NEXT:       func.call @__revert(%c0_i256_3, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc10)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc10)
// CHECK-NEXT:     } loc(#loc10)
// CHECK-NEXT:     %c32_i256_1 = arith.constant 32 : i256 loc(#loc10)
// CHECK-NEXT:     %11 = arith.muli %9, %c32_i256_1 : i256 loc(#loc10)
// CHECK-NEXT:     %12 = arith.addi %8, %11 : i256 loc(#loc10)
// CHECK-NEXT:     %13 = llvm.inttoptr %12 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:     %14 = llvm.load %13 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc10)
// CHECK-NEXT:     return %14 : i256 loc(#loc13)
// CHECK-NEXT:   } loc(#loc7)
// CHECK-NEXT:   func.func @ui_1d_16.0(%arg0: i256 loc(unknown), %arg1: i256 loc(unknown)) -> i256 attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc15)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc15)
// CHECK-NEXT:     llvm.store %arg0, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc15)
// CHECK-NEXT:     %c1_i256_0 = arith.constant 1 : i256 loc(#loc16)
// CHECK-NEXT:     %1 = llvm.alloca %c1_i256_0 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc16)
// CHECK-NEXT:     llvm.store %arg1, %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc16)
// CHECK-NEXT:     %2 = llvm.load %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc1)
// CHECK-NEXT:     %3 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc17)
// CHECK-NEXT:     %c3_i256 = arith.constant 3 : i256 loc(#loc1)
// CHECK-NEXT:     %4 = arith.cmpi uge, %3, %c3_i256 : i256 loc(#loc1)
// CHECK-NEXT:     scf.if %4 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc1)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc1)
// CHECK-NEXT:       %9 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %9 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc1)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc1)
// CHECK-NEXT:       %10 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       llvm.store %c50_i256, %10 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       %c0_i256_1 = arith.constant 0 : i256 loc(#loc1)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc1)
// CHECK-NEXT:       %c2_i256 = arith.constant 2 : i256 loc(#loc1)
// CHECK-NEXT:       func.call @__revert(%c0_i256_1, %c24_i256, %c2_i256) : (i256, i256, i256) -> () loc(#loc1)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc1)
// CHECK-NEXT:     } loc(#loc1)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc1)
// CHECK-NEXT:     %5 = arith.muli %3, %c32_i256 : i256 loc(#loc1)
// CHECK-NEXT:     %6 = arith.addi %2, %5 : i256 loc(#loc1)
// CHECK-NEXT:     %7 = llvm.inttoptr %6 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:     %8 = llvm.load %7 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc1)
// CHECK-NEXT:     return %8 : i256 loc(#loc18)
// CHECK-NEXT:   } loc(#loc14)
// CHECK-NEXT:   func.func @ui_1d_16(%arg0: i256 loc(unknown), %arg1: i256 loc(unknown)) -> i256 attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, runtime, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc15)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc15)
// CHECK-NEXT:     llvm.store %arg0, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc15)
// CHECK-NEXT:     %c1_i256_0 = arith.constant 1 : i256 loc(#loc16)
// CHECK-NEXT:     %1 = llvm.alloca %c1_i256_0 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc16)
// CHECK-NEXT:     llvm.store %arg1, %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc16)
// CHECK-NEXT:     %2 = llvm.load %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc1)
// CHECK-NEXT:     %3 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc17)
// CHECK-NEXT:     %c3_i256 = arith.constant 3 : i256 loc(#loc1)
// CHECK-NEXT:     %4 = arith.cmpi uge, %3, %c3_i256 : i256 loc(#loc1)
// CHECK-NEXT:     scf.if %4 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc1)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc1)
// CHECK-NEXT:       %9 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %9 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc1)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc1)
// CHECK-NEXT:       %10 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       llvm.store %c50_i256, %10 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:       %c0_i256_1 = arith.constant 0 : i256 loc(#loc1)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc1)
// CHECK-NEXT:       %c0_i256_2 = arith.constant 0 : i256 loc(#loc1)
// CHECK-NEXT:       func.call @__revert(%c0_i256_1, %c24_i256, %c0_i256_2) : (i256, i256, i256) -> () loc(#loc1)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc1)
// CHECK-NEXT:     } loc(#loc1)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc1)
// CHECK-NEXT:     %5 = arith.muli %3, %c32_i256 : i256 loc(#loc1)
// CHECK-NEXT:     %6 = arith.addi %2, %5 : i256 loc(#loc1)
// CHECK-NEXT:     %7 = llvm.inttoptr %6 : i256 to !llvm.ptr<1> loc(#loc1)
// CHECK-NEXT:     %8 = llvm.load %7 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc1)
// CHECK-NEXT:     return %8 : i256 loc(#loc18)
// CHECK-NEXT:   } loc(#loc14)
// CHECK-NEXT:   func.func @ui_2d_36(%arg0: i256 loc(unknown), %arg1: i256 loc(unknown)) -> i256 attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, runtime, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc8)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc8)
// CHECK-NEXT:     llvm.store %arg0, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc8)
// CHECK-NEXT:     %c1_i256_0 = arith.constant 1 : i256 loc(#loc9)
// CHECK-NEXT:     %1 = llvm.alloca %c1_i256_0 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc9)
// CHECK-NEXT:     llvm.store %arg1, %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc9)
// CHECK-NEXT:     %2 = llvm.load %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc10)
// CHECK-NEXT:     %3 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc11)
// CHECK-NEXT:     %c4_i256 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:     %4 = arith.cmpi uge, %3, %c4_i256 : i256 loc(#loc10)
// CHECK-NEXT:     scf.if %4 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc10)
// CHECK-NEXT:       %15 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c4_i256_2 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc10)
// CHECK-NEXT:       %16 = llvm.inttoptr %c4_i256_2 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c50_i256, %16 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c0_i256_3 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc10)
// CHECK-NEXT:       %c0_i256_4 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       func.call @__revert(%c0_i256_3, %c24_i256, %c0_i256_4) : (i256, i256, i256) -> () loc(#loc10)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc10)
// CHECK-NEXT:     } loc(#loc10)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc10)
// CHECK-NEXT:     %5 = arith.muli %3, %c32_i256 : i256 loc(#loc10)
// CHECK-NEXT:     %6 = arith.addi %2, %5 : i256 loc(#loc10)
// CHECK-NEXT:     %7 = llvm.inttoptr %6 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:     %8 = llvm.load %7 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc10)
// CHECK-NEXT:     %9 = llvm.load %1 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc12)
// CHECK-NEXT:     %c3_i256 = arith.constant 3 : i256 loc(#loc10)
// CHECK-NEXT:     %10 = arith.cmpi uge, %9, %c3_i256 : i256 loc(#loc10)
// CHECK-NEXT:     scf.if %10 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc10)
// CHECK-NEXT:       %15 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c4_i256_2 = arith.constant 4 : i256 loc(#loc10)
// CHECK-NEXT:       %c50_i256 = arith.constant 50 : i256 loc(#loc10)
// CHECK-NEXT:       %16 = llvm.inttoptr %c4_i256_2 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       llvm.store %c50_i256, %16 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:       %c0_i256_3 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc10)
// CHECK-NEXT:       %c0_i256_4 = arith.constant 0 : i256 loc(#loc10)
// CHECK-NEXT:       func.call @__revert(%c0_i256_3, %c24_i256, %c0_i256_4) : (i256, i256, i256) -> () loc(#loc10)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc10)
// CHECK-NEXT:     } loc(#loc10)
// CHECK-NEXT:     %c32_i256_1 = arith.constant 32 : i256 loc(#loc10)
// CHECK-NEXT:     %11 = arith.muli %9, %c32_i256_1 : i256 loc(#loc10)
// CHECK-NEXT:     %12 = arith.addi %8, %11 : i256 loc(#loc10)
// CHECK-NEXT:     %13 = llvm.inttoptr %12 : i256 to !llvm.ptr<1> loc(#loc10)
// CHECK-NEXT:     %14 = llvm.load %13 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc10)
// CHECK-NEXT:     return %14 : i256 loc(#loc13)
// CHECK-NEXT:   } loc(#loc7)
// CHECK-NEXT:   func.func @malloc_1d_47() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, runtime, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc6)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %c96_i256 = arith.constant 96 : i256 loc(#loc6)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc6)
// CHECK-NEXT:     %1 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     %2 = llvm.load %1 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc6)
// CHECK-NEXT:     %3 = arith.addi %2, %c96_i256 : i256 loc(#loc6)
// CHECK-NEXT:     %c18446744073709551615_i256 = arith.constant 18446744073709551615 : i256 loc(#loc6)
// CHECK-NEXT:     %4 = arith.cmpi ugt, %3, %c18446744073709551615_i256 : i256 loc(#loc6)
// CHECK-NEXT:     %5 = arith.cmpi ult, %3, %2 : i256 loc(#loc6)
// CHECK-NEXT:     %6 = arith.ori %4, %5 : i1 loc(#loc6)
// CHECK-NEXT:     scf.if %6 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc6)
// CHECK-NEXT:       %14 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %14 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc6)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc6)
// CHECK-NEXT:       %15 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       llvm.store %c65_i256, %15 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:       %c0_i256_1 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc6)
// CHECK-NEXT:       %c0_i256_2 = arith.constant 0 : i256 loc(#loc6)
// CHECK-NEXT:       func.call @__revert(%c0_i256_1, %c24_i256, %c0_i256_2) : (i256, i256, i256) -> () loc(#loc6)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc6)
// CHECK-NEXT:     } loc(#loc6)
// CHECK-NEXT:     %c64_i256_0 = arith.constant 64 : i256 loc(#loc6)
// CHECK-NEXT:     %7 = llvm.inttoptr %c64_i256_0 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     llvm.store %3, %7 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     %8 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %9 = llvm.load %8 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     %10 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc6)
// CHECK-NEXT:     %11 = llvm.load %10 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc6)
// CHECK-NEXT:     %12 = llvm.getelementptr %11[%9] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc6)
// CHECK-NEXT:     %13 = llvm.inttoptr %2 : i256 to !llvm.ptr<1> loc(#loc6)
// CHECK-NEXT:     "llvm.intr.memcpy"(%13, %12, %c96_i256) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc6)
// CHECK-NEXT:     llvm.store %2, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc6)
// CHECK-NEXT:     return loc(#loc5)
// CHECK-NEXT:   } loc(#loc5)
// CHECK-NEXT:   func.func @malloc_2d_60() attributes {llvm.linkage = #llvm.linkage<private>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality, runtime, state_mutability = #NonPayable} {
// CHECK-NEXT:     %c1_i256 = arith.constant 1 : i256 loc(#loc4)
// CHECK-NEXT:     %0 = llvm.alloca %c1_i256 x i256 : (i256) -> !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %c128_i256 = arith.constant 128 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %1 = llvm.inttoptr %c64_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %2 = llvm.load %1 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %3 = arith.addi %2, %c128_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %4 = arith.cmpi ugt, %3, %c18446744073709551615_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %5 = arith.cmpi ult, %3, %2 : i256 loc(#loc4)
// CHECK-NEXT:     %6 = arith.ori %4, %5 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %6 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_18 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c0_i256_19 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_18, %c24_i256, %c0_i256_19) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_0 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %7 = llvm.inttoptr %c64_i256_0 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %3, %7 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c96_i256 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_1 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %8 = llvm.inttoptr %c64_i256_1 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %9 = llvm.load %8 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %10 = arith.addi %9, %c96_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_2 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %11 = arith.cmpi ugt, %10, %c18446744073709551615_i256_2 : i256 loc(#loc4)
// CHECK-NEXT:     %12 = arith.cmpi ult, %10, %9 : i256 loc(#loc4)
// CHECK-NEXT:     %13 = arith.ori %11, %12 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %13 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_18 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c0_i256_19 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_18, %c24_i256, %c0_i256_19) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_3 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %14 = llvm.inttoptr %c64_i256_3 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %10, %14 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %15 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %16 = llvm.load %15 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %17 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %18 = llvm.load %17 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %19 = llvm.getelementptr %18[%16] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %20 = llvm.inttoptr %9 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%20, %19, %c96_i256) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %21 = llvm.inttoptr %2 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %9, %21 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c32_i256 = arith.constant 32 : i256 loc(#loc4)
// CHECK-NEXT:     %22 = arith.addi %2, %c32_i256 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_4 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_5 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %23 = llvm.inttoptr %c64_i256_5 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %24 = llvm.load %23 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %25 = arith.addi %24, %c96_i256_4 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_6 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %26 = arith.cmpi ugt, %25, %c18446744073709551615_i256_6 : i256 loc(#loc4)
// CHECK-NEXT:     %27 = arith.cmpi ult, %25, %24 : i256 loc(#loc4)
// CHECK-NEXT:     %28 = arith.ori %26, %27 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %28 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_18 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c0_i256_19 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_18, %c24_i256, %c0_i256_19) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_7 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %29 = llvm.inttoptr %c64_i256_7 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %25, %29 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %30 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %31 = llvm.load %30 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %32 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %33 = llvm.load %32 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %34 = llvm.getelementptr %33[%31] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %35 = llvm.inttoptr %24 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%35, %34, %c96_i256_4) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %36 = llvm.inttoptr %22 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %24, %36 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c64_i256_8 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %37 = arith.addi %2, %c64_i256_8 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_9 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_10 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %38 = llvm.inttoptr %c64_i256_10 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %39 = llvm.load %38 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %40 = arith.addi %39, %c96_i256_9 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_11 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %41 = arith.cmpi ugt, %40, %c18446744073709551615_i256_11 : i256 loc(#loc4)
// CHECK-NEXT:     %42 = arith.cmpi ult, %40, %39 : i256 loc(#loc4)
// CHECK-NEXT:     %43 = arith.ori %41, %42 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %43 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_18 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c0_i256_19 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_18, %c24_i256, %c0_i256_19) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_12 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %44 = llvm.inttoptr %c64_i256_12 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %40, %44 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %45 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %46 = llvm.load %45 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %47 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %48 = llvm.load %47 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %49 = llvm.getelementptr %48[%46] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %50 = llvm.inttoptr %39 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%50, %49, %c96_i256_9) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %51 = llvm.inttoptr %37 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %39, %51 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %c96_i256_13 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %52 = arith.addi %2, %c96_i256_13 : i256 loc(#loc4)
// CHECK-NEXT:     %c96_i256_14 = arith.constant 96 : i256 loc(#loc4)
// CHECK-NEXT:     %c64_i256_15 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %53 = llvm.inttoptr %c64_i256_15 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %54 = llvm.load %53 {alignment = 1 : i64} : !llvm.ptr<1> -> i256 loc(#loc4)
// CHECK-NEXT:     %55 = arith.addi %54, %c96_i256_14 : i256 loc(#loc4)
// CHECK-NEXT:     %c18446744073709551615_i256_16 = arith.constant 18446744073709551615 : i256 loc(#loc4)
// CHECK-NEXT:     %56 = arith.cmpi ugt, %55, %c18446744073709551615_i256_16 : i256 loc(#loc4)
// CHECK-NEXT:     %57 = arith.cmpi ult, %55, %54 : i256 loc(#loc4)
// CHECK-NEXT:     %58 = arith.ori %56, %57 : i1 loc(#loc4)
// CHECK-NEXT:     scf.if %58 {
// CHECK-NEXT:       %c0_i256 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256 = arith.constant 35408467139433450592217433187231851964531694900788300625387963629091585785856 : i256 loc(#loc4)
// CHECK-NEXT:       %67 = llvm.inttoptr %c0_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c35408467139433450592217433187231851964531694900788300625387963629091585785856_i256, %67 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c4_i256 = arith.constant 4 : i256 loc(#loc4)
// CHECK-NEXT:       %c65_i256 = arith.constant 65 : i256 loc(#loc4)
// CHECK-NEXT:       %68 = llvm.inttoptr %c4_i256 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       llvm.store %c65_i256, %68 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:       %c0_i256_18 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       %c24_i256 = arith.constant 24 : i256 loc(#loc4)
// CHECK-NEXT:       %c0_i256_19 = arith.constant 0 : i256 loc(#loc4)
// CHECK-NEXT:       func.call @__revert(%c0_i256_18, %c24_i256, %c0_i256_19) : (i256, i256, i256) -> () loc(#loc4)
// CHECK-NEXT:       func.call @".unreachable"() : () -> () loc(#loc4)
// CHECK-NEXT:     } loc(#loc4)
// CHECK-NEXT:     %c64_i256_17 = arith.constant 64 : i256 loc(#loc4)
// CHECK-NEXT:     %59 = llvm.inttoptr %c64_i256_17 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %55, %59 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     %60 = llvm.mlir.addressof @calldatasize : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %61 = llvm.load %60 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     %62 = llvm.mlir.addressof @ptr_calldata : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %63 = llvm.load %62 {alignment = 32 : i64} : !llvm.ptr<ptr<3>> loc(#loc4)
// CHECK-NEXT:     %64 = llvm.getelementptr %63[%61] : (!llvm.ptr<3>, i256) -> !llvm.ptr<3>, i8 loc(#loc4)
// CHECK-NEXT:     %65 = llvm.inttoptr %54 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     "llvm.intr.memcpy"(%65, %64, %c96_i256_14) <{isVolatile = false}> : (!llvm.ptr<1>, !llvm.ptr<3>, i256) -> () loc(#loc4)
// CHECK-NEXT:     %66 = llvm.inttoptr %52 : i256 to !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %54, %66 {alignment = 1 : i64} : i256, !llvm.ptr<1> loc(#loc4)
// CHECK-NEXT:     llvm.store %2, %0 {alignment = 32 : i64} : !llvm.ptr<i256> loc(#loc4)
// CHECK-NEXT:     return loc(#loc3)
// CHECK-NEXT:   } loc(#loc3)
// CHECK-NEXT:   func.func private @__personality() -> i32 attributes {llvm.linkage = #llvm.linkage<external>, passthrough = ["nofree", "null_pointer_is_valid"], personality = @__personality} loc(#loc)
// CHECK-NEXT: } loc(#loc)
// CHECK-NEXT: #loc1 = loc({{.*}}:4:11)
// CHECK-NEXT: #loc3 = loc({{.*}}:13:2)
// CHECK-NEXT: #loc4 = loc({{.*}}:14:4)
// CHECK-NEXT: #loc5 = loc({{.*}}:10:2)
// CHECK-NEXT: #loc6 = loc({{.*}}:11:4)
// CHECK-NEXT: #loc7 = loc({{.*}}:6:2)
// CHECK-NEXT: #loc8 = loc({{.*}}:6:17)
// CHECK-NEXT: #loc9 = loc({{.*}}:6:41)
// CHECK-NEXT: #loc10 = loc({{.*}}:7:11)
// CHECK-NEXT: #loc11 = loc({{.*}}:7:13)
// CHECK-NEXT: #loc12 = loc({{.*}}:7:16)
// CHECK-NEXT: #loc13 = loc({{.*}}:7:4)
// CHECK-NEXT: #loc14 = loc({{.*}}:3:2)
// CHECK-NEXT: #loc15 = loc({{.*}}:3:17)
// CHECK-NEXT: #loc16 = loc({{.*}}:3:38)
// CHECK-NEXT: #loc17 = loc({{.*}}:4:13)
// CHECK-NEXT: #loc18 = loc({{.*}}:4:4)
// CHECK-EMPTY:
