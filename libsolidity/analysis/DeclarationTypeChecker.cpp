...
void DeclarationTypeChecker::endVisit(VariableDeclaration const& _variable)
{
	...
	if (typeLoc == DataLocation::Stack && type->isDynamicallySized()) {
		// TODO: Run scripts/error_codes.py --fix
		m_errorReporter.fatalTypeError(0000_error, _variable.location(), "Dynamically sized stack data location is not implemented.");
	}
